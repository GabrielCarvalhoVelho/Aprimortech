...
/**
 * Worker para sincronização automática de clientes em background
...
class ClienteSyncWorker(
    ...
    companion object {
        private const val TAG = "ClienteSyncWorker"
        private const val WORK_NAME = "cliente_sync_work"
        private const val PERIODIC_WORK_NAME = "cliente_periodic_sync"

        /**
         * Executa sincronização imediata (one-time)
         */
        fun syncNow(context: Context) {
            val workRequest = OneTimeWorkRequestBuilder<ClienteSyncWorker>()
                .setConstraints(
                    Constraints.Builder()
        ...
        fun schedulePeriodicSync(context: Context) {
            val periodicWorkRequest = PeriodicWorkRequestBuilder<ClienteSyncWorker>(
    ...
    override suspend fun doWork(): Result {
        return try {
            ...
            // Sincroniza clientes pendentes
            val clientesSincronizados = app.clienteRepository.sincronizarClientesPendentes()

            // Atualiza cache local com dados do Firebase
            app.clienteRepository.sincronizarComFirebase()

            Log.d(TAG, "✅ Sincronização concluída: $clientesSincronizados clientes sincronizados")

            // Notifica sucesso
            setProgress(
                workDataOf(
                    "sincronizados" to clientesSincronizados,
